...
// Streaming cache writer for thresholded 1-bit pixels, MSB first.
//
...
struct PixelCache {
  ...
  bool begin(const std::string& cachePath, int w, int h, int ox, int oy, int maxBlockDstRows) {
    ...
    // High width bit identifies the focused 1-bit cache layout. Legacy 2-bit
    // cache files have it clear and are regenerated by ImageBlock.
    uint16_t w16 = static_cast<uint16_t>(w) | 0x8000;
