...
bool ReaderActivity::isEpubFile(const std::string& path) { return FsHelpers::hasEpubExtension(path); }
...
void ReaderActivity::onEnter() {
  ...
  if (isTxtFile(initialBookPath)) {
  ...
  } else if (isEpubFile(initialBookPath)) {
    ...
    LOG_ERR("READER", "Unsupported file type: %s", initialBookPath.c_str());
    onGoBack();
  }
}
